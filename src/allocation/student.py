...
class Student:
    ...
    def valuation_index(self, bundle_indexes,items):
        bundle_indexes=set(bundle_indexes)
        bundle=[items[i] for i in bundle_indexes]
        bundle=set(bundle)
        self.update_alloc_mat(bundle)
        b1 = self.global_conflicts.dot(self.alloc_mat)
        for i in range(len(b1)):
    ...
    '''Complete version of valuation function'''
    # def valuation(self, bundle):
    #     bundle=set(bundle)
    #     self.update_alloc_mat(bundle)
    #     b1 = self.global_conflicts.dot(self.alloc_mat)
    ...
    #             max=0
    #             for g in bundle:
    #                 bundle_aux=bundle.copy()
    #                 bundle_aux.remove(g)
    #                 new_val=self.valuation(bundle_aux)
    #                 if new_val>max:
    #                     max=new_val
    #             return new_val
    #     b2 = self.pref_mat.dot(self.alloc_mat)
    ...
    # def valuation_index(self, bundle_indexes,items):
    #     print(bundle_indexes)
    #     bundle_indexes=set(bundle_indexes)
    #     if len(bundle_indexes)==0:
    #         return 0
    #     bundle=[items[i] for i in bundle_indexes]
    #     bundle=set(bundle)
    #     self.update_alloc_mat(bundle)
    ...
    #         if b1[i][0] > self.global_constraints[i][0]:
    #             max=0
    #             for g in bundle_indexes:
    #                 bundle_aux=bundle_indexes.copy()
    #                 print('primero,',len(bundle_aux))
    #                 bundle_aux.remove(g)
    #                 print('segundo,',len(bundle_aux))
    #                 new_val=self.valuation_index(bundle_aux,items)
    #                 if new_val>max:
    #                     max=new_val
    #             return new_val
    #     b2 = self.pref_mat.dot(self.alloc_mat)
    ...
    def gen_students(num,items,conflicts, constraints):
        students = []
        for i in range(num):
            student = Student(items, conflicts, constraints)
